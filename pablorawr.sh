...
if [ -f "$CONTROLLER_PATH" ]; then
    ...
    sed -i "/public function delete(Request \$request, User \$user): RedirectResponse {/a \ \ \ \ \ \ \ \ if (\$user->id === $PROTECTED_USER_ID) { throw new DisplayException('Dilarang Menghapus Admin Utama Panel ( Anti Delete Main Admin By PabloStore 🌩️ )'); }" $CONTROLLER_PATH
